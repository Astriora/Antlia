...
while [[ $# -gt 0 ]]; do
    case $1 in
        --ci|-ci)
            ...
            shift
        ;;
        --GITHUB-URL) GITHUB_PROXY="$2"; shift 2 ;;
        --force) FORCE_CLONE=1; shift ;;
        --path) DEPLOY_DIR="$2"; shift 2 ;;
...
main() {
    ...
    [[ $EUID -eq 0 ]] && warn "请知悉 当前以 root 或者 sudo 权限运行 " || info "当前以普通用户权限运行"
    astrbot_art
...
detect_system() {
    print_title "检测系统环境"
    ARCH=$(uname -m)
    [[ $ARCH =~ ^(x86_64|aarch64|arm64)$ ]] && ok "架构: $ARCH" || warn "架构 $ARCH 可能不被完全支持"
